...
export function registerCoinGeckoTools(server, cache, limiter) {
    // ── coingecko.get_price ───────────────────────────────────────────────────
    server.tool("coingecko.get_price", "Get current prices and market data for one or more coins from CoinGecko", {
        ids: z
            .array(z.string())
            .min(1)
    ...
    server.tool("coingecko.get_market_chart", "Get historical market chart data (price, market cap, volume) for a coin from CoinGecko", {
        ...
        try {
            ...
            if (!data) {
                ...
                data = await fetchJson(`${BASE_URL}/coins/${encodeURIComponent(args.id)}/market_chart?${params}`, { headers: getHeaders() });
    ...
    server.tool("coingecko.get_trending", "Get currently trending coins on CoinGecko (top 7 searched in last 24h)", {}, async () => {
        try {
            limiter.assertLimit(PROVIDER);
    ...
    server.tool("coingecko.get_token_info", "Get detailed token information including community and developer data from CoinGecko", {
        ...
        try {
            ...
            if (!data) {
                ...
                data = await fetchJson(`${BASE_URL}/coins/${encodeURIComponent(args.id)}?${params}`, { headers: getHeaders() });
    ...
    // ── coingecko.search ──────────────────────────────────────────────────────
    server.tool("coingecko.search", "Search for coins, exchanges, and categories on CoinGecko", {
