...
function makeCacheKey(tool, params) {
    const sorted = Object.fromEntries(Object.entries(params)
...
export function registerDefiLlamaTools(server, cache, limiter) {
    // ── defillama.get_protocol ────────────────────────────────────────────────
    server.tool("defillama.get_protocol", "Get protocol TVL, chain breakdown, and metadata from DefiLlama", {
        protocol_slug: z
            .string()
    ...
    server.tool("defillama.get_yields", "Get yield pool data from DefiLlama, optionally filtered by chain, project, or stablecoin", {
        ...
        try {
            ...
            if (!data) {
                limiter.recordUsage(PROVIDER);
                const raw = await fetchJson("https://yields.llama.fi/pools");
                let pools = raw.data ?? [];
                if (args.chain) {
    ...
    // ── defillama.get_fees ────────────────────────────────────────────────────
    server.tool("defillama.get_fees", "Get protocol fee data from DefiLlama for a given time period", {
        ...
        try {
            ...
            if (!data) {
                limiter.recordUsage(PROVIDER);
                data = await fetchJson(`https://api.llama.fi/summary/fees/${encodeURIComponent(args.protocol_slug)}?dataType=dailyFees`);
                cache.set(key, data, PROVIDER, 300);
            }
            return toolResponse(data);
        }
    ...
    server.tool("defillama.get_chain_tvl", "Get TVL data for all chains or historical TVL for a specific chain", {
        chain: z
            .string()
            .optional()
        ...
        try {
            ...
            if (!data) {
                limiter.recordUsage(PROVIDER);
                if (args.chain) {
                    data = await fetchJson(`https://api.llama.fi/v2/historicalChainTvl/${encodeURIComponent(args.chain)}`);
                }
                else {
    ...
    server.tool("defillama.get_stablecoins", "Get stablecoin data including market caps and prices, optionally filtered by chain", {
        chain: z
            .string()
            .optional()
            ...
            if (!data) {
                ...
                if (args.chain) {
                    ...
                    assets = assets.filter((a) => {
                        const chainBalances = a["chainBalances"];
                        if (!chainBalances)
                            return false;
    ...
    server.tool("defillama.get_bridges", "Get bridge data — all bridges or a specific bridge by ID, optionally filtered by chain", {
        bridge_id: z
            .number()
            .int()
            .optional()
        ...
        try {
            ...
            if (!data) {
                ...
                if (args.bridge_id !== undefined) {
                    ...
                    const raw = await fetchJson("https://bridges.llama.fi/bridges?includeChains=true");
    ...
    server.tool("defillama.get_token_prices", "Get current or historical token prices from DefiLlama coins API", {
        coins: z
            .array(z.string())
            .min(1)
            .describe("Array of coin identifiers in 'chain:address' format (e.g. ['ethereum:0xabcd...', 'coingecko:bitcoin'])"),
        timestamp: z
            .number()
            .int()
            .optional()
