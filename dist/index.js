...
async function main() {
    const server = new McpServer({ name: "crypto-research", version: "1.0.0" }, {
        capabilities: {
            tools: {},
            resources: {},
        },
    ...
    server.resource("budget-status", "budget://status", { description: "Current rate limit usage per provider" }, async () => {
    ...
    process.stderr.write("[crypto-mcp-router] Starting crypto-research MCP server v1.0.0\n");
    process.stderr.write(`[crypto-mcp-router] Cache DB: ${process.env["CACHE_DB_PATH"] ?? "./cache.db"}\n`);
    process.stderr.write(`[crypto-mcp-router] CoinGecko API key: ${process.env["COINGECKO_API_KEY"] ? "set" : "not set (free tier)"}\n`);
