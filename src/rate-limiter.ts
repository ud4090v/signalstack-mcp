...
export class RateLimiter {
  private configs: Map<string, ProviderRateLimitConfig> = new Map();
  private slidingWindowRequests: Map<string, number[]> = new Map();
  private creditUsage: Map<string, UsageRecord[]> = new Map();
  ...
  checkLimit(provider: string): boolean {
    ...
    if (isRateLimitConfig(config)) {
      ...
      const requests = (this.slidingWindowRequests.get(provider) ?? []).filter(
        (ts) => ts > windowStart
      );
  ...
  recordUsage(provider: string, credits = 1): void {
    const config = this.configs.get(provider);
    if (!config) return;
  ...
  assertLimit(provider: string): void {
    ...
    if (!this.checkLimit(provider)) {
      if (isRateLimitConfig(config)) {
        ...
        throw new Error(
          `Rate limit exceeded for ${provider}: ${config.maxRequests} requests per ${config.windowMs}ms. ` +
      ...
      if (isCreditLimitConfig(config)) {
        const used = this.getMonthlyCreditsUsed(provider);
        throw new Error(
          `Monthly credit limit exceeded for ${provider}: ${used}/${config.maxCredits} credits used this month.`
        );
      }
    }
  }
  ...
  private getMonthlyCreditsUsed(provider: string): number {
    const now = Date.now();
    ...
    const usage = (this.creditUsage.get(provider) ?? []).filter(
      (r) => r.timestamp >= monthStartMs && r.timestamp <= now
    );
